# app/services/booking_chain.py
from app.models import Guest, Room, Booking
from sqlalchemy.orm import Session
from sqlalchemy import and_
from app.crud import GuestRepository, RoomRepository
...
class RoomExistsHandler(Handler):
    def handle(self, data):
        ...
        # Оставляем Repository как было
        room_repo = RoomRepository(self.db)
        room = room_repo.get(room_number=room_number)
        if not room:
            raise ValueError(f"Комната с номером {room_number} не найдена.")
        return super().handle(data)
